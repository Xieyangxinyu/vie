...
def train(X, y, M, lifetime_max, delta, importance = True):
    ...
    return X_bd_all, X, history, w_trees

def evaluate(X, y, X_test, M, history, w_trees):
    """
    Sweeps through Mondrian kernels with all lifetime in [0, lifetime_max]. This can be used to (1) construct a Mondrian
    feature map with lifetime lifetime_max, to (2) find a suitable lifetime (inverse kernel width), or to (3) compare
    Mondrian kernel to Mondrian forest across lifetimes.
...
def simulate_proj_H(x_train, y_train, x_test, y_test, M, lifetime_max, delta, H = None):
    ...
    _, _, history, w_trees = train(x_train_transformed, y_train, M, lifetime_max, delta)

    _, y_hat_test = evaluate(x_train_transformed, y_train, x_test_transformed, M, history, w_trees)
...
def train_and_evaluate(x_train, y_train, x_test, y_test, M, lifetime_max, delta):
    ...
    X_bd_all, X, history, w_trees = train(x_train, y_train, M, lifetime_max, delta)
    y_hat_train, y_hat_test = evaluate(x_train, y_train, x_test, M, history, w_trees)

    mse = mean_squared_error(y_test, y_hat_test)
    return mse, X_bd_all, X, history, w_trees, y_hat_train

def estimate_H(x_train, y_train, X_bd_all, M, X, history, w_trees, y_hat_train):
    dim_in = X.shape[1]
    n = X.shape[0]
    importance = []
    for dim in range(dim_in):
        try:
            ...
            _, y_hat_eval = evaluate(x_train, y_train, x_eval, M, history, w_trees)
...
def experiment(x_train, y_train, x_test, y_test, M, lifetime_max, delta, H, full = True):
    mse, X_bd_all, X, history, w_trees, y_hat_train = train_and_evaluate(x_train, y_train, x_test, y_test, M, lifetime_max, delta)
    H_itr = estimate_H(x_train, y_train, X_bd_all, M, X, history, w_trees, y_hat_train)
    if full:
...
def evaluate_all_lifetimes(X, y, X_test, y_test, M, lifetime_max, delta):
    ...
    results = {'times': list_times, 'mse': list_forest_error_test}
    return results